"""
TXT Loader — reads the raw text (encoding-aware).

Structural parsing into the element schema happens in the Parse step
(txt_parser), so the lifecycle stays UPLOAD → LOADED → (Parse) → EXTRACTED and
...
def load(file_path: str) -> dict:
    ...
    metadata = build_doc_metadata(file_path, 1, "txt", parser_name="text")
